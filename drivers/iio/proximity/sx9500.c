			      ...
			      struct sx9500_data *data)
{
	...
	if (client->irq <= 0) {
		gpio = devm_gpiod_get_index(dev, SX9500_GPIO_INT, 0);
		if (IS_ERR(gpio)) {
		...
		if (ret < 0)
			dev_err(dev, "gpio irq set input failed\n");
		else
			client->irq = gpiod_to_irq(gpio);
	}

	data->gpiod_rst = devm_gpiod_get_index(dev, SX9500_GPIO_RESET, 0);
	...
	gpiod_direction_output(data->gpiod_rst, 1);
}
