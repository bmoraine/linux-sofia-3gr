...
static struct device_state_pm_state lsm303c_acc_pm_states[] = {
	{ .name = "disable", }, /* D3 */
	{ .name = "enable", }, /* D0 */
};

static int lsm303c_acc_set_pm_state(struct device *dev,
...
static struct device_state_pm_state *lsm303c_acc_get_initial_state(
		struct device *dev)
{
	return &lsm303c_acc_pm_states[0];
}

static struct device_state_pm_ops lsm303c_acc_pm_ops = {
	.set_state = lsm303c_acc_set_pm_state,
	.get_initial_state = lsm303c_acc_get_initial_state,
};

DECLARE_DEVICE_STATE_PM_CLASS(lsm303c_acc);
#endif

#ifdef CONFIG_OF
int lsm303c_acc_power_on(struct device *dev)
{
...
int lsm303c_acc_power_off(struct device *dev)
{
	struct lsm303c_acc_status *stat = dev_get_drvdata(dev);
	struct device_pm_platdata *pm_platdata = stat->pdata->pm_platdata;
	int err = device_state_pm_set_state_by_name(dev,
...
static int lsm303_acc_init(struct device *dev)
{
	struct lsm303c_acc_status *stat = dev_get_drvdata(dev);
	struct device_pm_platdata *pm_platdata = stat->pdata->pm_platdata;
...
static void lsm303_acc_exit(void)
{
	return;
}
...
skip_pinctrl:
	...
	acc_pdata->init = lsm303_acc_init;
	acc_pdata->exit = lsm303_acc_exit;
	acc_pdata->power_on = lsm303c_acc_power_on;
	acc_pdata->power_off = lsm303c_acc_power_off;
		...
		struct pinctrl_state *state)
{
	...
	if (!pdata) {
		dev_err(dev,
			"Unable to retrieve lsm303c_acc platform data\n");
		return -EINVAL;
	}
									...
									int len)
{
	...
	if (stat->use_smbus) {
		if (len == 1) {
			...
			"%s:ret=0x%02x, len:%d, command=0x%02x, buf[0]=0x%02x\n",
				__func__, ret, len, cmd , buf[0]);
#endif
		} else if (len > 1) {
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"%s:ret=%d, len:%d, command=0x%02x, value=0x%02x\n",
				__func__, ret, len, reg , value);
#endif
			return ret;
		} else if (len > 1) {
...
static int lsm303c_acc_input_init(struct lsm303c_acc_status *stat)
{
	...
	stat->input_dev->name = "lsm303dlhc_acc";
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		stat->pdata = devm_kzalloc(&client->dev,
			sizeof(struct lsm303c_acc_platform_data), GFP_KERNEL);
		if (NULL == stat->pdata) {
			err = -ENOMEM;
			dev_err(&client->dev,
		...
		memcpy(stat->pdata, &default_lsm303c_acc_pdata,
			sizeof(struct lsm303c_acc_platform_data));
...
{
#ifndef CONFIG_PLATFORM_DEVICE_PM_VIRT
	int ret = device_state_pm_add_class(&lsm303c_acc_pm_class);
	if (ret) {
		pr_err("%s: %s: ERROR adding %s pm class\n",
				LSM303C_ACC_DEV_NAME, __func__,
				lsm303c_acc_pm_class.name);
		return ret;
	}
#endif
