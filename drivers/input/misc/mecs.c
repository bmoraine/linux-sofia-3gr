/*
 * Adaptations were made to original mecs.c with
...
static long ecs_ctrl_ioctl(struct file *file, unsigned int cmd,
			unsigned long arg);
...
static short ecompass_delay;


static struct input_dev *ecs_data_device;

static const struct file_operations ecs_ctrl_fops = {
	.owner		= THIS_MODULE,
	.open		= ecs_ctrl_open,
...
static int __init ecompass_init(void)
{
	...
	res = device_create_file(ecs_ctrl_device.this_device,
				&dev_attr_ecs_ctrl);
	if (res) {
