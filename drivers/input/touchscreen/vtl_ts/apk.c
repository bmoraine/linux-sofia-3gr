...
** Function define
*****************************************************************************/
static int apk_i2c_transfer(struct i2c_client *client,
	unsigned char i2c_addr, unsigned char len,
	unsigned char *buf, unsigned char rw)
{
...
static int apk_write(struct file *file, const char __user *buff,
	size_t count, loff_t *offp)
{
	...
	if (frame_data[0] == APK_ITOUCH) {
		...
		switch (frame_data[1]) {
			...
			if (frame_data[4])
				enable_irq(
				ts_object->config_info->irq_number);
			else
				disable_irq(
				ts_object->config_info->irq_number);
		...
		pr_debug("CMD: %s,count = %d\n", frame_data, count);
		switch (frame_data[1]) {
			case	SHELL_CHECKSUM_CMD:{
					chip_get_checksum(client,
...
static int apk_read(struct file *file,
	...
	if (frame_data[0] == APK_ITOUCH) {
		...
		switch (frame_data[1]) {
			...
			case	READ_CHIP_CMD:{
					...
					ret = apk_i2c_transfer(client,
						frame_data[2],
						frame_data[3], frame_data, 1);
				} break;

			case	READ_CHECKSUM_CMD:{
					ret = chip_get_checksum(client,
						&bin_checksum, &fw_checksum);
...
const struct file_operations apk_fops = {
	.owner	 = THIS_MODULE,
	.open    = apk_open,
	.release = apk_close,
	.write   = apk_write,
