...
enum iommu_interrupt {
	IOMMU_INTERRUPT_PAGE_FAULT = 0x01, /**< A page fault occurred */
	IOMMU_INTERRUPT_READ_BUS_ERROR = 0x02 /**< A bus read error occurred */
};
...
static unsigned long *rockchip_page_entry(unsigned long *sent,
				       unsigned long iova)
{
...
static void rockchip_iommu_disable_stall(void __iomem *base)
{
	...
	for (i = 0; i < IOMMU_REG_POLL_COUNT_FAST; ++i) {
		...
		if (0 == (mmu_status & IOMMU_STATUS_BIT_STALL_ACTIVE))
			break;
...
static void rockchip_iommu_page_fault_done(void __iomem *base,
					const char *dbgname)
{
...
static inline bool rockchip_iommu_raw_reset(void __iomem *base)
{
	...
	__raw_writel(IOMMU_COMMAND_HARD_RESET, base + IOMMU_REGISTER_COMMAND);
...
static void rockchip_iommu_set_ptbase(void __iomem *base,
				   unsigned long pgd)
{
...
{
#ifdef CONFIG_X86
	clflush_cache_range(vastart, vaend - vastart);
#else
...
static irqreturn_t rockchip_iommu_irq(int irq, void *dev_id)
{
	...
	for (i = 0; i < data->num_res_mem; i++) {
		...
		if (rawstat & IOMMU_INTERRUPT_READ_BUS_ERROR)
			dev_err(data->iommu, "bus error occurred at %pad\n",
				&fault_address);
...
int rockchip_iommu_tlb_invalidate(struct device *dev)
{
	...
	if (rockchip_is_iommu_active(data)) {
		...
		for (i = 0; i < data->num_res_mem; i++) {
			...
			base = data->res_bases[i];

			ret = rockchip_iommu_zap_tlb_without_stall(base);
			if (ret) {
				dev_err(dev->archdata.iommu, "(%s) %s failed\n",
					data->dbgname, __func__);
				spin_unlock_irqrestore(&data->data_lock, flags);
...
static phys_addr_t rockchip_iommu_iova_to_phys(struct iommu_domain *domain,
					    dma_addr_t iova)
{
	struct rockchip_iommu_domain *priv = domain->priv;
	unsigned long *entry;
	unsigned long flags;
	phys_addr_t phys = 0;
...
static int rockchip_iommu_map(struct iommu_domain *domain,
			   ...
			   size_t size, int prot)
{
	...
	offset = rockchip_lv1ent_offset(iova);

	entry = rockchip_section_entry(priv->pgtable, iova);

	pent = rockchip_alloc_lv2entry(entry, iova, &priv->lv2entcnt[offset]);
	if (!pent)
		ret = -ENOMEM;
	else
		ret = rockchip_lv2set_page(pent, paddr, size,
					&priv->lv2entcnt[offset]);
...
{
	struct rockchip_iommu_domain *priv;

	priv = kzalloc(sizeof(*priv), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

/* iommu use 2 level pagetable,
...
static int rockchip_get_res_num(struct platform_device *pdev,
			     unsigned int type)
{
	int num = 0;
	int i;
...
static int rockchip_iommu_probe(struct platform_device *pdev)
{
	...
	data->num_res_mem = rockchip_get_res_num(pdev, IORESOURCE_MEM);
	...
	data->num_res_irq = rockchip_get_res_num(pdev, IORESOURCE_IRQ);
...
static struct platform_driver rockchip_iommu_driver = {
...
static int __init rockchip_iommu_init_driver(void)
{
	...
	return platform_driver_register(&rockchip_iommu_driver);
}
