...
static bool get_gc0310_lock_awb(struct v4l2_subdev *sd)
{
	int value;
	bool status;
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	gc_read_reg(client, GC_8BIT, GC0310_AWB_LOCK_REG, &value);

	/* set bit to 0 when set awb lock on */
	status = !(value & GC0310_AWB_LOCK_BITS) ? true : false;
	return status;
}

static bool get_gc0310_lock_ae(struct v4l2_subdev *sd)
{
	int value;
	bool status;
	struct i2c_client *client = v4l2_get_subdevdata(sd);

	gc_read_reg(client, GC_8BIT, GC0310_AE_LOCK_REG, &value);

	/* set bit to 0 when set ae lock on */
	status = !(value & GC0310_AE_LOCK_BITS) ? true : false;
	return status;
}
		...
		const struct i2c_device_id *id)
{
	...
	dev->get_lock_awb =  get_gc0310_lock_awb;
	dev->get_lock_ae  =  get_gc0310_lock_ae;
