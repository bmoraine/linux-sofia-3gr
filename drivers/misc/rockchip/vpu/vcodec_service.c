			      ...
			      unsigned long arg)
{
	...
	case COMPAT_VPU_IOC_SECVM_CMD: {
#if defined(CONFIG_MOBILEVISOR_VDRIVER_PIPE)
		/* IMC: send a VVPU command to secure VM */
		struct vvpu_secvm_cmd vvpu_cmd;

		if (copy_from_user(&vvpu_cmd, compat_ptr((compat_uptr_t)arg),
				sizeof(vvpu_cmd)))
			return -EFAULT;
		...
		if (copy_to_user(compat_ptr((compat_uptr_t)arg), &vvpu_cmd,
				sizeof(vvpu_cmd)))
			return -EFAULT;
#else
		ret = -EFAULT;
	...
	case COMPAT_VPU_IOC_SECVM_PP_CMD: {
#if defined(CONFIG_MOBILEVISOR_VDRIVER_PIPE)
		/* IMC: send a VVPU command to secure VM */
		struct vvpu_secvm_cmd vvpu_pp_cmd;
		if (copy_from_user(&vvpu_pp_cmd, compat_ptr((compat_uptr_t)arg),
				sizeof(vvpu_pp_cmd)))
			return -EFAULT;
		/*pr_err("ioctl VPU_IOC_SECVM_PP_CMD vop is %d\n",
						vvpu_pp_cmd.payload[1]);*/
		vpu_service_power_on(pservice);
		vvpu_pp_call(pservice->dev, &vvpu_pp_cmd);
		if (copy_to_user(compat_ptr((compat_uptr_t)arg), &vvpu_pp_cmd,
				sizeof(vvpu_pp_cmd)))
			return -EFAULT;
#else
		ret = -EFAULT;
