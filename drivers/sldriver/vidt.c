...
#define ALIGN_CODE_SECT(x) \
__attribute__((aligned (4096), section (x), noinline))
...
typedef struct {
	uint16 limit;
	uint32 base;
} __packed  IA32_GDTR, IA32_IDTR, IA32_LDTR;
...
} __packed IA32_IDT_GATE_DESCRIPTOR;

typedef struct {
	uint32 addr_orig;
	uint32 addr_vidt;
} __packed IA32_IDT_REDIRECT_TABLE;

typedef struct {
	uint64 addr_orig;
	uint64 addr_vidt;
} __packed IA32e_IDT_REDIRECT_TABLE;
...
} __packed IA32e_IDT_GATE_DESCRIPTOR;

typedef struct {
	uint16 limit;
	uint64 base;
} __packed IA32e_GDTR, IA32e_IDTR, IA32e_LDTR;
...
typedef struct {
	...
	struct {
		...
		uint32 default_size:1;	/* 0 = 16-bit & 1 = 32-bit segment */
		uint32 granularity:1;
		uint32 base_address_31_24:8;
	} hi;
} __packed IA32_CODE_SEGMENT_DESCRIPTOR;
...
#endif
/*initialize before use in vidt_setup*/
unsigned long ptr_core_state DATA_SECT(".vidtd") = 1;
/*alloc and init during vidt setup*/
unsigned long secs_la ALIGN_DATA_SECT(".vidtd") = 0x0;

#ifdef __x86_64__
extern int request_sl_service(uint32_t service_leaf, uint64_t param_a,
				  uint64_t param_b, uint64_t S, uint64_t D);
#else
extern int request_sl_service(uint32_t service_leaf, uint32_t param_a,
...
int init_view_map_list(struct file *file)
{
	...
	if (!file)
		return VIDT_FAIL;
	pfd_view_map = kmalloc(sizeof(struct view_list), GFP_KERNEL);
...
int clean_ta_view(void *priv_data)
{
	...
	list_for_each_entry_safe(pos, temp_node, &temp_head->list, list) {
		if (VIDT_PRIV_MAGIC == pos->view_data.magic) {
#ifdef __x86_64__
			...
			pr_err("MAGIC 0x%lx doesn't match 0x%lx\n",
...
extern void my_handler_non_arch_end(void);
extern void my_handler(void);
extern void my_handler_arch_end(void);
...
int get_vidt_code_pages(uint8_t **vidt_buf, uint32_t *vidt_size)
{
	...
	/* -------------------------------------------------------- */
	/*alloc and clear pages for per_cpu_vidt_stub (my_handler) */
	isr_size = ((unsigned int)my_handler_non_arch_end -
			(unsigned int)my_handler_arch_end);
	per_cpu_vidt_stub = calloc(1, isr_size * 256);
	if (per_cpu_vidt_stub == NULL) {
		pr_err("%s:%d: Failed allocating memory\n", __func__, __LINE__);
		ret = -1;
	...
	memcpy((char *)((unsigned int)per_cpu_vidt_stub),
		   (char *)my_handler, isr_size * 21);
	/*copy non-arch handlers 21-255 */
	for (i = 21; i < 256; i++) {
		memcpy((char *)((unsigned int)per_cpu_vidt_stub +
				(isr_size * i)), (char *)my_handler_arch_end,
			   isr_size);
	}
	...
	memcpy((char *)((unsigned int)per_cpu_enter_eresume_code),
		   (char *)enter_eresume_code,
		   (unsigned int)enter_eresume_code_end -
...
void print_idt(void *desc)
{
	...
	for (i = 0; i < 256; i++) {
		KdPrint(PRINT_DEBUG,
			("Gate Type 0x%x Selector:Address 0x%x:0x%x \n",
			 Idt[i].GateType, Idt[i].Selector,
#ifdef __x86_64__
			 ((uint64_t) Idt[i].OffsetHigh << 32) | (Idt[i].
								 OffsetMid <<
								 16)
			 | (Idt[i].OffsetLow)));
#else
	...
	for (i = 0; i < 256; i++) {
		...
		redirect[cpuindex][i].addr_orig =
			(((uint64_t) Idt[i].OffsetHigh << 32) |
			 ((uint64_t) Idt[i].OffsetMid << 16) | (Idt[i].OffsetLow));
#else
		redirect[cpuindex][i].addr_orig =
			(Idt[i].OffsetHigh << 16) | (Idt[i].OffsetLow);
#endif
		redirect[cpuindex][i].addr_vidt =
			(unsigned long)codebase + isr_size * i;
...
void InstallExceptionHandler(void *desc,
				 uint32 ExceptionIndex, uint64 HandlerAddr,
				 uint32 GateType, uint32 dpl)
{
#ifdef __x86_64__
	IA32e_IDT_GATE_DESCRIPTOR *Idt = (IA32e_IDT_GATE_DESCRIPTOR *) desc;
#else
...
void update_vidt(uint32 cpuindex, void *desc)
{
	...
	for (i = 0; i < 256; i++) {
		...
		if (IA32_IDT_GATE_TYPE_TASK == Idt[i].GateType) {
			KdPrint(PRINT_DEBUG,
				("Gate Type - task gate - skipping "));
			continue;
		}

		InstallExceptionHandler((void *)Idt, i,
					redirect[cpuindex][i].addr_vidt,
		...
		KdPrint(PRINT_DEBUG,
			("Gate Type 0x%x Selector:Address 0x%x:0x%x ",
			 ...
			 (Idt[i].OffsetHigh << 16) | (Idt[i].OffsetLow)));
#endif
		KdPrint(PRINT_DEBUG, ("original Address 0x%x\n",
					  redirect[cpuindex][i].addr_orig));
	}
}
...
void update_vidt_special_handlers(uint32 cpuindex, void *param)
{
	...
	KdPrint(PRINT_ALWAYS, ("Gate Type 0x%x Selector:Address 0x%x:0x%x\n",
				   Idt[i].GateType, Idt[i].Selector,
#ifdef __x86_64__
				   ((uint64_t) Idt[i].OffsetHigh << 32) |
				   ((uint64_t) Idt[i].OffsetMid << 16) |
	...
	KdPrint(PRINT_ALWAYS, ("original Address 0x%x\n",
				   redirect[cpuindex][i].addr_orig));
	...
	InstallExceptionHandler((void *)Idt,
				i, redirect[cpuindex][i].addr_vidt,
				IA32_GATE_TYPE_INTR, IA32_DPL3);
	KdPrint(PRINT_DEBUG, ("Gate Type 0x%x Selector:Address 0x%x:0x%x\n",
				  Idt[i].GateType, Idt[i].Selector,
#ifdef __x86_64__
				  ((uint64_t) Idt[i].OffsetHigh << 32) |
				  ((uint64_t) Idt[i].OffsetMid << 16) |
...
int setup_vidt(void)
{
	...
	sl_info = kmalloc(sizeof(hsec_sl_param_t), GFP_KERNEL);
	if ((void *)sl_info == NULL) {
	...
	vIDT_info = kmalloc(sizeof(hsec_vIDT_param_t), GFP_KERNEL);
	if ((void *)vIDT_info == NULL) {
		KdPrint(PRINT_ALWAYS,
			("kmalloc failed %s: %d\n", __func__, __LINE__));
		SAFE_FREE(sl_info);
		return SL_EUNKNOWN;
	...
	pr_info("setup_vidt loaded at %p\n", (void *)setup_vidt);
	/*1 page for SECS la */
	secs_la = (unsigned long)kmalloc(0x1000, GFP_KERNEL);
	if ((void *)secs_la == NULL) {
		KdPrint(PRINT_ALWAYS,
			("kmalloc failed %s: %d\n", __func__, __LINE__));
		SAFE_FREE(sl_info);
		SAFE_FREE(vIDT_info);
...
#ifdef __x86_64__
	redirect_ptr =
		kmalloc(sizeof(IA32e_IDT_REDIRECT_TABLE) * 256 * MAX_CORES,
			GFP_KERNEL);
#else
	redirect_ptr =
		kmalloc(sizeof(IA32_IDT_REDIRECT_TABLE) * 256 * MAX_CORES,
			GFP_KERNEL);
#endif
	...
	ptr_core_state =
		(unsigned long)kmalloc(sizeof(PER_CORE_STATE) * MAX_CORES,
				   GFP_KERNEL);

	if ((void *)ptr_core_state == NULL) {
		KdPrint(PRINT_ALWAYS,
			("kmalloc failed %s: %d\n", __func__, __LINE__));
		SAFE_FREE(secs_ptr);
	...
	for_each_online_cpu(cpuindex) {
		...
		IA32_IDTR idt_desc;
#endif
		hsec_patch_info_t *stub_patch, *test_code_patch;
		hsec_patch_info_t *enter_eresume_code_patch, *exit_code_patch;

		test_code_patch = stub_patch = enter_eresume_code_patch = NULL;
		exit_code_patch = NULL;
		...
		smp_call_function_single(cpuindex, read_gdtr_fp, (void *)&gdtr,
					 1);

		secs_ptr->pcd[cpuindex].gdtr = gdtr.base;

		smp_call_function_single(cpuindex, read_ldtr_fp, (void *)&ldtr,
					 1);
		secs_ptr->pcd[cpuindex].ldtr = ldtr.base;

		smp_call_function_single(cpuindex, read_idtr_fp, (void *)&idtr,
					 1);
		...
		/*alloc and clear pages for per_cpu_vidt_stub (my_handler) */
		isr_size = ((unsigned long)my_handler_non_arch_end -
				(unsigned long)my_handler_arch_end);
		...
		/*alloc and clear pages for AEX Exit stub code */
		if (((unsigned long)test_code_end - (unsigned long)test_code) <
			0x1000) {
			per_cpu_test_code = kmalloc(0x1000, GFP_KERNEL);
		} else {
			per_cpu_test_code =
				kmalloc((unsigned long)test_code_end -
		...
		/*alloc and clear pages for sync exit stub code */
		if (((unsigned long)exit_code_end - (unsigned long)exit_code) <
			0x1000) {
			per_cpu_exit_code = kmalloc(0x1000, GFP_KERNEL);
		} else {
			per_cpu_exit_code =
				kmalloc((unsigned long)exit_code_end -
		...
		if (((unsigned long)enter_eresume_code_end -
			...
			per_cpu_enter_eresume_code =
				kmalloc(0x1000, GFP_KERNEL);
		} else {
			per_cpu_enter_eresume_code =
				kmalloc((unsigned long)enter_eresume_code_end -
		...
		per_cpu_r0stack_pages =
			kmalloc(0x1000 * NUM_STACK_PAGES, GFP_KERNEL);
		...
		memset((void *)per_cpu_enter_page, 0, 0x1000);
		memset((void *)per_cpu_exit_page, 0, 0x1000);
		...
		KdPrint(PRINT_ALWAYS, ("vidt is at 0x%lx\n", (uint64) vidt));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_vidt_stub is at 0x%lx size 0x%x\n",
			 (uint64) per_cpu_vidt_stub, isr_size * 256));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_test_code is at 0x%lx \n",
			 (uint64) per_cpu_test_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_exit_code is at 0x%lx \n",
			 (uint64) per_cpu_exit_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_enter_eresume_code is at 0x%lx \n",
			 (uint64) per_cpu_enter_eresume_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_r0stack_pages start at 0x%lx \n",
			 (uint64) per_cpu_r0stack_pages));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_enter_pages start at 0x%lx \n",
			 (uint64) per_cpu_enter_page));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_exit_page start at 0x%lx \n",
			 (uint64) per_cpu_exit_page));
#else

		KdPrint(PRINT_ALWAYS, ("vidt is at 0x%x\n", (uint32) vidt));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_vidt_stub is at 0x%x size 0x%x\n",
			 (uint32) per_cpu_vidt_stub, isr_size * 256));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_test_code is at 0x%x\n",
			 (uint32) per_cpu_test_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_exit_code is at 0x%x\n",
			 (uint32) per_cpu_exit_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_enter_eresume_code is at 0x%x\n",
			 (uint32) per_cpu_enter_eresume_code));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_r0stack_pages start at 0x%x\n",
			 (uint32) per_cpu_r0stack_pages));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_enter_pages start at 0x%x\n",
			 (uint32) per_cpu_enter_page));
		KdPrint(PRINT_ALWAYS,
			("per_cpu_exit_page start at 0x%x\n",
		...
		memset(vIDT_info, 0, sizeof(hsec_vIDT_param_t));
		/*copy handler code */
		...
		for (i = 21; i < 256; i++) {
			memcpy((char *)((unsigned long)per_cpu_vidt_stub +
					(isr_size * i)),
				   (char *)my_handler_arch_end, isr_size);
		}
		...
		/* 6 patches - cpuindex, ptr_core_state,
		 * 2 secs ptr, secs scv, exit_page*/
		test_code_patch =
			kmalloc(NUM_AEX_CODE_PATCHES * sizeof(hsec_patch_info_t),
				GFP_KERNEL);
		...
		test_code_offset =
			(unsigned long)test_code_ptr_core_state_patch -
			(unsigned long)test_code;
#if 0
		memcpy((char *)((unsigned long)per_cpu_test_code +
				test_code_offset + 0x1),
		...
		/*special value to	signify that hypersim should patch by itself. */
		test_code_patch[3].val = (unsigned long)~0x0;
		...
		enter_eresume_code_patch =
			kmalloc(NUM_ENT_RES_CODE_PATCHES *
				sizeof(hsec_patch_info_t), GFP_KERNEL);
		...
		enter_eresume_code_offset =
			(unsigned long)enter_eresume_code_cpuindex -
			(unsigned long)enter_eresume_code;
#if 0
		memcpy((char *)((unsigned long)per_cpu_enter_eresume_code +
		...
		enter_eresume_code_patch[0].offset = enter_eresume_code_offset +
			MOV_OPCODE_SIZE;
		enter_eresume_code_patch[0].type = PATCH_TYPE_CORE_ID;

		enter_eresume_code_offset =
			(unsigned long)enter_eresume_code_cmp_patch1 -
			(unsigned long)enter_eresume_code;
		enter_eresume_code_patch[1].val = (unsigned long)~0x0;
		enter_eresume_code_patch[1].offset = enter_eresume_code_offset +
			MOV_OPCODE_SIZE;
		enter_eresume_code_patch[1].type = PATCH_TYPE_SECS_SCV_UN;

		enter_eresume_code_offset =
			(unsigned long)enter_eresume_code_cmp_patch2 -
			(unsigned long)enter_eresume_code;
		enter_eresume_code_patch[2].val = (unsigned long)~0x0;
		enter_eresume_code_patch[2].offset = enter_eresume_code_offset +
			MOV_OPCODE_SIZE;
		...
		enter_eresume_code_offset =
			(unsigned long)enter_eresume_code_enter_page_patch -
			(unsigned long)enter_eresume_code;
		enter_eresume_code_patch[9].val =
			(unsigned long)per_cpu_enter_page;
		enter_eresume_code_patch[9].offset =
			enter_eresume_code_offset + MOV_OPCODE_SIZE;
		...
		ADD_REGION(region_map, per_cpu_enter_eresume_code, 1,
			   enter_eresume_code_patch, NUM_ENT_RES_CODE_PATCHES,
			   region_size);
		...
		exit_code_patch =
			kmalloc(NUM_EXIT_CODE_PATCHES * sizeof(hsec_patch_info_t),
				GFP_KERNEL);
		...
		KdPrint(PRINT_ALWAYS, ("CPU %x IDT lim is 0x%hx\n",
					   cpuindex, idtr.limit));
		KdPrint(PRINT_ALWAYS, ("CPU %x IDT base is 0x%x\n",
					   cpuindex, idtr.base));
		KdPrint(PRINT_ALWAYS, ("==============\n"));
...
#ifdef __x86_64__
		vidt_start =
			(IA32e_IDT_GATE_DESCRIPTOR *) ((unsigned long)vidt +
						   page_offset);
#else
		vidt_start =
			(IA32_IDT_GATE_DESCRIPTOR *) ((unsigned long)vidt +
						  page_offset);
#endif

		/*save os orig handlers and update our */
		/*handlers in cpu specific redirect table */
		read_idt(cpuindex, (unsigned long)per_cpu_vidt_stub,
			 (void *)idt_start);
		KdPrint(PRINT_DEBUG, ("==============\n"));
		...
		stub_patch = kmalloc(256 * 2 * sizeof(hsec_patch_info_t),
					 GFP_KERNEL);
		if (stub_patch == NULL) {
			KdPrint(PRINT_ALWAYS, ("kmalloc failed %s: %d\n",
						   __func__, __LINE__));
		...
		for (i = 0; i < 256; i++) {
			...
			stub_patch[i * 2].offset = i * isr_size +
				...
				(unsigned long)my_handler + MOV_OPCODE_SIZE;
			...
			case EENTER_VECTOR:
			case ERESUME_VECTOR:
				stub_patch[i * 2 + 1].val =
					(unsigned long)per_cpu_enter_eresume_code;
				break;
			case EEXIT_VECTOR:
				stub_patch[i * 2 + 1].val =
					(unsigned long)per_cpu_exit_code;
				break;
			default:
				stub_patch[i * 2 + 1].val =
		...
		core_state[cpuindex].redirect_addr =
			(unsigned long)(redirect_ptr +
					(cpuindex * 256 *
					 sizeof(IA32e_IDT_REDIRECT_TABLE)));
		/*(unsigned long)redirect[cpuindex]; */
		core_state[cpuindex].redirect_size =
			sizeof(IA32e_IDT_REDIRECT_TABLE) * 256;
		...
		core_state[cpuindex].redirect_addr =
			(unsigned long)(redirect_ptr +
					(cpuindex * 256 *
					 sizeof(IA32_IDT_REDIRECT_TABLE)));
		/*(unsigned long)redirect[cpuindex]; */
		core_state[cpuindex].redirect_size =
			sizeof(IA32_IDT_REDIRECT_TABLE) * 256;
		core_state[cpuindex].vep_max = 0;	/*unused remove */
		core_state[cpuindex].vep_tos = 0;	/*unused remove */

#endif
		KdPrint(PRINT_ALWAYS,
			("core state %d = 0x%x 0x%x 0x%x 0x%x\n", cpuindex,
		...
		memcpy((char *)ptr_core_state,
			   (char *)core_state, sizeof(PER_CORE_STATE) * MAX_CORES);
...
/*		  smp_call_function_single(cpuindex, ia32_load_idtr, &Idtr, 1);*/
		...
		ADD_REGION(region_map, per_cpu_vidt_stub,
			   ((isr_size * 256) >> 12) + 1, stub_patch,
			   n_stub_patches, (isr_size * 256));
		i = 0;
		while ((per_cpu_vidt_stub + i * 0x1000) <
			   (per_cpu_vidt_stub + (isr_size * 256))) {
			set_page_exec((unsigned long)(per_cpu_vidt_stub +
							  i * 0x1000));
			i++;
		}
		...
		smp_call_function_single(cpuindex, reg_vIDT, (void *)vIDT_info,
					 1);

		KdPrint(PRINT_ALWAYS,
			("finished registering vidt for cpu  %u, running on cpu %d\n",
					cpuindex, smp_processor_id()));
		/*verify */
#ifdef __x86_64__
		smp_call_function_single(cpuindex, ia32e_read_idtr, &idt_desc,
					 1);
#else
		smp_call_function_single(cpuindex, ia32_read_idtr, &idt_desc,
					 1);
#endif
		/*	   smp_call_function_single(cpuindex, print_gdt, NULL, 0); */
		...
		/* For test only - after XO permission,
		 * enabling this code should cause
		 * write protection violation in the hypersim.*/
#if 0
		memcpy((char *)((unsigned long)per_cpu_vidt_stub
				+ EENTER_VECTOR * isr_size + 0xF),
...
fail_msg:
	temp_ptr_core_state = (void *)(unsigned long)ptr_core_state;
	put_online_cpus();
