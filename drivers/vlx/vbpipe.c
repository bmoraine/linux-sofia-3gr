 ...
 ****************************************************************
 *
 *  Component: VLX VBPIPE Communication driver
 *
...
/* device flags */
#define WAIT_ONLY_ON_EMPTY   0x0001  /* The read waits only on empty buffer */
#define NO_SLEEP_WRITE       0x0002

/* ioctls */
#define VBPIPE_IOC_DRVMODE  0x1234

static ssize_t ex_write_sleep (struct file* file, const char __user* buf, size_t count, loff_t* ppos);
static ssize_t ex_write_no_sleep (struct file* file, const char __user* buf, size_t count, loff_t* ppos);
...
ex_handshake (ExDev* ex_dev, int server)
{
    ...
    if (server) {
	my_state   = (volatile int *)&ex_dev->s_link->s_state;
	peer_state =  ex_dev->s_link->c_state;
    } else {
	my_state   = (volatile int *)&ex_dev->c_link->c_state;
          ...
          size_t count, loff_t* ppos)
{
    ...
    if (ex_dev->flags & NO_SLEEP_WRITE)
        return ex_write_no_sleep(file, buf, count, ppos);
    else
        return ex_write_sleep(file, buf, count, ppos);
}
     ...
     */
    static ssize_t
ex_write_sleep (struct file* file, const char __user* buf,
          size_t count, loff_t* ppos)
{
    unsigned int minor   = iminor(file->f_path.dentry->d_inode);
    ExDev*       ex_dev  = &ex_devs[minor];
    ...
    static ssize_t
ex_write_no_sleep (struct file* file, const char __user* buf,
          size_t count, loff_t* ppos)
{
    ...
    while ((cnt = (count - done))) {
        ...
        cring->c_wait = 1;
        wmb();
        /* If there is no room in the buffer or the BE drv is not up return
         * error. This prevents cpu soft-lockup happening when BE driver stops
         * working */
        if (RING_P_ROOM(cring, ex_dev->c_size) == 0 ||
            clink->s_state != NK_DEV_VLINK_ON ) {
            if (!done)
...
#if defined(BULLSEYE_HOSTTEST) || defined (EX_IOCTL)
    /*
...
static const struct file_operations ex_fops = {
    ...
    .poll	= ex_poll,
    .llseek	= no_llseek,
#if defined(EX_IOCTL) // not defined now
    .ioctl	= ex_ioctl,
#endif
};
...
{
    return single_open (file, ex_seq_proc_show, PDE_DATA(inode));
}
...
void hosttest_func_vbpipe(void);
#endif
...
ex_proc_init (void)
{
    ...
    ent = proc_create("nk/vbpipe", 0, NULL, &ex_proc_fops);
    if (!ent) {
    ...
    ex_proc_exists = 1;
#if defined(BULLSEYE_HOSTTEST)
    hosttest_add_test("vbpipe", hosttest_func_vbpipe);
#endif
    return 0;
}
...
static _Bool ex_param(NkDevVlink* vlink, ExDev* ex_dev, _Bool server,
...
#define EX_NOT_END(ch) ((ch) != ';' && (ch) != '\0')
    ...
    s = (const char*) nkops.nk_ptov((NkPhAddr) vlink->s_info);
    if (EX_NOT_END (*s)) {
        if (*s != 'a')
            goto error_2;
...
void hosttest_func_vbpipe(void)
{
    ...
    const char __user* test_buf;
    size_t test_count;
    loff_t* test_ppos;
		poll_table* test_wait;
		...
		ExDev* test_ex_dev;
		unsigned test_minor;
		NkDevVlink* test_vlink;
		const char* test_location;
		
    ex_write_no_sleep (test_file, test_buf, test_count, test_ppos);
		ex_poll(test_file, test_wait);
		ex_ioctl(test_inode, test_file, test_cmd, test_addr);		
    ex_proc_exit();
		ex_dev_destroy(test_ex_dev, test_minor);
    ex_vlink_module_cleanup();
    ex_syntax_error(test_vlink, test_location);
    vbpipe_module_exit();
}
#endif
