 ...
 *
 * Nov 05 2012: Update API function call for kernel 3.0
 ****************************************************************
 */
...
#define TX_READY_PACKETS 0x6		/* minimum threshhold to trigger resume notify callback */
/*----- Shared data types -----*/
...
typedef struct {
    ...
    unsigned	count_unsent;	/* unsent packets count before xirq raised in producer*/
    unsigned	count_freed;	/* freed packets count in consumer/producer */
} vmq_xx;
...
vmq_xx_msg_free (vmq_xx* xx, void* msg, _Bool signal)
{
    ...
    if (head->unsafe_stopped || signal || (xx->count_freed == (xx->config.msg_count > TX_READY_PACKETS ? TX_READY_PACKETS : xx->config.msg_count )) ) {
	if( head->unsafe_stopped && (xx->count_freed == 0))
			xx->count_freed = 1;
	if( xx->count_freed == (xx->config.msg_count > TX_READY_PACKETS ? TX_READY_PACKETS : xx->config.msg_count ) )
			xx->count_freed = 0;
...
static void vmq_tx_resume_notify (vmq_tx* tx);
		     ...
		     unsigned* data_offset, _Bool nonblocking)
{
    ...
    if(VMQ_UNSAFE_C_PENDING(&tx->xx) >= (tx->xx.config.msg_count /2))
...
{
    volatile int*	my_state   = (volatile int *) &tx->xx.vlink->c_state;
    volatile int*	peer_state = (volatile int *) &tx->xx.vlink->s_state;
...
{
    volatile int*	my_state   = (volatile int*) &rx->xx.vlink->s_state;
    volatile int*	peer_state = (volatile int*) &rx->xx.vlink->c_state;
    _Bool		need_sysconf = false;
...
vmq_tx_notify (vmq_tx* tx)
{
    ...
    static void
vmq_tx_resume_notify (vmq_tx* tx)
{
     if( tx->xx.count_freed >= (tx->xx.config.msg_count > TX_READY_PACKETS ? TX_READY_PACKETS : tx->xx.config.msg_count )) {
		vmq_link_t* link = container_of (tx, vmq_link_t, tx);
   		tx->xx.count_freed = 0;
		if (link->callbacks->resume_notify) {
			link->callbacks->resume_notify (link);
			} 
     	}
}
...
{
    seq_printf (seq,
		"%s: %6x %4x %6x %4x %2d %2d %2d %3d %3d %5u %5u %3u %3u\n",
		name,
		xx->config.msg_count,  xx->config.msg_max,
...
{
    return single_open (file, vmq_proc_show, PDE_DATA (inode));
}
	       ...
	       const _Bool is_frontend)
{
    ...
    snprintf (links->proc_name, len, "nk/vmq.%s-%ce", vlink_name,
	      is_frontend ? 'f' : 'b');
    links->proc = proc_create_data(links->proc_name, 0, NULL,
		    &vmq_proc_fops, links);
    if (!links->proc) {
		 ...
		 const vmq_xx_config_t* rx_config)
{
    ...
    if (rx_vlink->s_info) {
	link2->public2.rx_s_info = (char*) nkops.nk_ptov ((NkPhAddr) rx_vlink->s_info);
	DTRACE ("rx_s_info '%s'\n", link2->public2.rx_s_info);
    }
    if (tx_vlink->s_info) {
	link2->public2.tx_s_info = (char*) nkops.nk_ptov ((NkPhAddr) tx_vlink->s_info);
	DTRACE ("tx_s_info '%s'\n", link2->public2.tx_s_info);
    }
    if (!tx_config) {
		   ...
		   _Bool is_frontend)
{
    ...
    *result = NULL;	/* Make sure it is NULL on error */
    ...
    if (!links) {
	ETRACE ("Could not allocate vmq_links descriptor.\n");
	diag = -ENOMEM;
	goto error;
    }
    links->public2.priv = priv;
    INIT_LIST_HEAD (&links->links);
    links->spinlock = __SPIN_LOCK_UNLOCKED(links->spinlock);
...
/****************change part*******************************/
_Bool
vmq_link_get_tx_free_buf_count (vmq_link_t* link, void* cookie)
{
  vmq_tx tx = link->tx;
  vmq_head* head = &tx.xx.pmem->head;
  tx.xx.safe_p_idx = tx.xx.pmem->head.unsafe_p_idx;
  if (vmq_xx_ring_is_full(&tx.xx)) {
 	* (nku32_f*)cookie = 0;
 	return false;
     }
  * (nku32_f*)cookie = tx.xx.config.msg_count -VMQ_UNSAFE_HEAD_SPACE(head) -VMQ_UNSAFE_C_PENDING(&tx.xx) - tx.xx.ss_checked_out ;
  return true;
}	
static signed
vmq_tx_return_msg (vmq_xx* xx, void** msg,
			   const struct list_head* ss_heads)
{
    vmq_head*		head = &xx->pmem->head;
    unsigned long	flags;
    VMQ_LOCK (xx->spinlock, flags);
	/* Sample unsafe value before checking it */
    xx->safe_p_idx = head->unsafe_p_idx -1 ; 
    if (VMQ_SAFE_HEAD_SPACE (xx) > 0) {
	const unsigned ss_number =
	    VMQ_TX_INDEX_SAFE_P_IDX (xx)->unsafe_ss_number;
	if (ss_number >= xx->config.msg_count ||
		!list_empty (ss_heads + ss_number)) {
	    vmq_xx_abort (xx, false);
	    VMQ_UNLOCK (xx->spinlock, flags);
	...
	*msg = VMQ_SHORT_SLOT (xx, ss_number);
	OTRACE ("msg %p p_idx %d/%d last_x_idx %d c_idx %d/%d\n", *msg,
		...
		head->unsafe_c_idx, xx->safe_c_idx);
	++xx->ss_checked_out;
	head->unsafe_p_idx = xx->safe_p_idx;
	VMQ_UNLOCK (xx->spinlock, flags);
	return 0;
    }
    VMQ_UNLOCK (xx->spinlock, flags);
    ...
    void
vmq_tx_discard_msg(vmq_link_t* link, void* cookie)
{
  void* msg;
  vmq_tx tx = link->tx;
  while ( tx.xx.count_unsent) {
	    if(!vmq_tx_return_msg (&tx.xx, &msg, tx.ss_heads))
			vmq_tx_return_msg_free (&tx, msg);
	    tx.xx.count_unsent --;		
  	}
}
/*----- Module description -----*/
EXPORT_SYMBOL(vmq_tx_discard_msg);
EXPORT_SYMBOL(vmq_link_get_tx_free_buf_count);
