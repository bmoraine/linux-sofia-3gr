...
{
    return single_open (file, _vrpc_proc_show, PDE_DATA(inode));
}
...
_vrpc_proc_init (void)
{
    ...
    ent = proc_create ("nk/vrpc", 0, NULL, &_vrpc_proc_fops);
    if (!ent) {
...
_vrpc_link_init (NkDevVlink* vlink, NkPhAddr plink)
{
    ...
    if (_vrpc_is_server(vrpc)) {
	vrpc->my.state   = (volatile int *) &vlink->s_state;
        if (vlink->s_info) {
            vrpc->my.info = (char*) nkops.nk_ptov((NkPhAddr) vlink->s_info);
  	}
	vrpc->peer.id    = vlink->c_id;
	vrpc->peer.state = (volatile int *) &vlink->c_state;
        if (vlink->c_info) {
            vrpc->peer.info = (char*) nkops.nk_ptov((NkPhAddr) vlink->c_info);
  	}
    } else {
	vrpc->my.state   = (volatile int *) &vlink->c_state;
        if (vlink->c_info) {
            vrpc->my.info = (char*) nkops.nk_ptov((NkPhAddr) vlink->c_info);
  	}
	vrpc->peer.id    = vlink->s_id;
	vrpc->peer.state = (volatile int *) &vlink->s_state;
        if (vlink->s_info) {
	    vrpc->peer.info = (char*) nkops.nk_ptov((NkPhAddr) vlink->s_info);
  	}
    }
...
#if defined(BULLSEYE_HOSTTEST)
    hosttest_add_test("vrpc", hosttest_func_vrpc);
#endif
...
#ifdef BULLSEYE_HOSTTEST
static void _vrpc_link_exit (vrpc_t* vrpc)
{
    return;
}
#else
...
void hosttest_func_vrpc(void)
{
    ...
    const char* test_name;
	  void* test_data;
		vrpc_call_t test_call;
		void* test_cookie;
		int test_direct;
		struct vrpc_t* test_s_vrpc;
		
    _vrpc_is_client(test_vrpc);
    _vrpc_null_call(test_vrpc);
    _vrpc_direct_call(test_vrpc);
    _vrpc_indirect_call(test_vrpc);
    
    _vrpc_proc_exit();
    vrpc_server_lookup(test_name,test_vrpc);
    vrpc_release(test_vrpc);
    vrpc_peer_id(test_vrpc);
    
    _vrpc_server_thread_wakeup(test_vrpc);
    _vrpc_server_thread(test_data);
    _vrpc_thread_stop(test_vrpc);
    vrpc_server_open(test_vrpc,test_call,test_cookie,test_direct);
    
    vrpc_close(test_vrpc);
    vrpc_info(test_s_vrpc);
    _vrpc_link_exit(test_vrpc);
    _vrpc_exit();
