...
/* Bit 5-6 of AFE_AUDIOINCTRL2_REG */
#define AFE_MASK_AUDIOINCTRL_VUMIC_MODE   0x6F

/* Bit 3 of AFE_AUDIOINCTRL3_REG*/
#define AFE_MASK_AUDIOINCTRL_HZ_VUMIC     0xF7
...
#define AFE_MASK_AUDIOINCTRL_MIC_LDO      0xCF

/* Bit 4 of AFE_AUDIOINCTRL2_REG */
		...
		*final_value = requested_value;
		...
		if (AFE_AUDIOINCTRL3_REG == reg) {
			...
			*final_value = (*final_value &
				AFE_MASK_AUDIOINCTRL_VUMIC_MODE);
			if ((0 == pmic_acc_det_data.mic_ldo_on)) {
				*final_value |=
			 ...
			 * vmic mode can be put off */
				*final_value = (*final_value &
					AFE_MASK_AUDIOINCTRL_VMIC_MODE);
				*final_value |= (0 <<
				AFE_SHIFT_AUDIOINCTRL_VMIC_MODE);
				*final_value = (*final_value &
						AFE_MASK_AUDIOINCTRL_VMIC_SELL);
				*final_value |=
					((pmic_acc_det_data.acc_det_param.
			...
			*final_value &= ~(1 << AFE_SHIFT_AUDIOINCTRL_MIC_LDO);
			...
			if ((0 == pmic_acc_det_data.mic_ldo_on)) {
				...
				*final_value = (*final_value &
						AFE_MASK_AUDIOINCTRL_VMIC_SELH);
				*final_value |=
					(((pmic_acc_det_data.acc_det_param.
			...
			*final_value |=
			...
			if (0 == (*final_value & ~(AFE_MASK_BCON_XB_ON))) {
				...
				*final_value |=
					(pmic_acc_det_data.
						acc_det_param.xb_mode <<
						AFE_SHIFT_BCON_XB_ON);
			}
		}
		result = 0;
	}
